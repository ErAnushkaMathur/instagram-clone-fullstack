...
If B's account is public → A follows B immediately. No request needed.
If B's account is private → a FollowRequest document is created with status: "pending".
Only B is authorized to accept or reject that request — enforced server-side, not just on the client.
On accept → the request status becomes "accepted" and an actual Follow relationship is created.
On reject → the request status becomes "rejected"; no follow relationship is created.
...
Anushka Mathur LinkedIn • GitHub
